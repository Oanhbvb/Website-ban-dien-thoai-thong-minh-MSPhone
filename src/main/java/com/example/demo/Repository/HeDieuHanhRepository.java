...
public interface HeDieuHanhRepository extends JpaRepository<HeDieuHanh, Integer> {
    ...
    @Query(value = "select h from  HeDieuHanh h order by h.id desc ")
     Page<HeDieuHanh> findAll(Pageable pageable);

    public Page<HeDieuHanh> findAllByTrangThai(Pageable pageable, int trangThai);

    @Query(value = "select h from  HeDieuHanh h where h.trangThai=1 order by h.id desc ")
    ...
    @Query(value = "SELECT p FROM HeDieuHanh p WHERE p.trangThai=:Status ORDER BY p.id DESC")
    List<HeDieuHanh> findAllBy(@Param("Status") int i);
}
